...
private:
    ...
    string createDigest(string& password, string& msg) {
        ...
        string ostring;
        ostring.assign(bptr->data, bptr->length - 1);
        BIO_free_all(b64);
        return ostring;
    }
    ...
    while (!context->isDone()) {
        try {
            ...
            if (portStr) {
                ...
                HADOOP_ASSERT(sock != - 1, string("problem creating socket: ") + strerror(errno));
                sockaddr_in addr;
                addr.sin_family = AF_INET;
                addr.sin_port = htons(toInt(portStr));
                addr.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
                HADOOP_ASSERT(connect(sock, (sockaddr*) &addr, sizeof(addr)) == 0, string("problem connecting command socket: ") + strerror(errno));
            }
            if (sock != -1) {
#ifdef WIN32
...
bool runTask(const Factory& factory) {
    try {
        ...
        if (portStr) {
            ...
            HADOOP_ASSERT(setbuf == 0, string("problem with setvbuf for inStream: ") + strerror(errno));
            setbuf = setvbuf(outStream, bufout, _IOFBF, bufsize);
            HADOOP_ASSERT(setbuf == 0, string("problem with setvbuf for outStream: ") + strerror(errno));
